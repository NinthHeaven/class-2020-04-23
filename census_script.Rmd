...
```{r load_raw, echo=FALSE, cache=TRUE}
...
# There are more than 5.7 million men.
...
```{r sc2, echo=FALSE, cache=FALSE}

x <- raw %>%
  mutate(death_date = ymd(paste(dyear, dmonth, "1", sep = "-"))) %>%
  select(death_date, death_age)

small <- x %>%
  sample_n(size = 10000)

ggplot(small, aes(death_age)) +
  geom_histogram()
```
...
seasons <- small %>%
  mutate(dmonth = month(death_date),
         season = ifelse(dmonth %in% c(10, 11, 12, 1, 2, 3), "Cold", "Warm"))

seasons %<>%
  group_by(age, season) %>%
  summarise(deaths = n(age))

ggplot(seasons, aes(death_date))
```
